...
mkdir -p "$out"
[ -x "$bin" ] || (cd "$root" && cargo build --release --example snapshot)
...
done < <(node -e "require('$manifest').forEach(function (t) { console.log(t.id); })")
...
ffmpeg -y -framerate 12 -i "$tmp/frames/f%03d.png" -movflags +faststart -pix_fmt yuv420p \
  -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" "$out/hero.mp4" >/dev/null 2>&1
ffmpeg -y -i "$tmp/hero.gif" -vframes 1 "$out/hero-poster.png" >/dev/null 2>&1
