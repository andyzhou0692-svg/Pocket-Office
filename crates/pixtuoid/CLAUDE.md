...
- **The eight-person minimum is a render-only lifecycle, not an active-agent cap.** `runtime::VisualCoworkers` always renders every Active or Waiting slot, then adds only enough idle coworkers to reach eight. It remembers exact slot generations that were working, so a task that settles to Idle walks out for `EXIT_GRACE_WINDOW` before a quiet coworker takes its seat; Vivian remains the persistent root. The lifecycle latches `Hidden` after a completed exit so a backward clock step cannot resurrect the avatar, then clears the latch only when that exact generation changes or becomes active again. The shared pose engine clears its cached exit and path when an id is rendered without `exiting_at`, then snapshots a fresh exit on the next departure. Replacing this lifecycle with `SceneState::agents.retain` makes avatars vanish between frames and bypasses both terminal and floating exit animation.
