...
│                       flattened SourceArgs flags + validate-pack's dir + init-pack's dest). Presenters live in main.rs
...
├── install/            multi-target (Claude + Codex + Reasonix + CodeWhale + opencode + Cursor + OpenClaw) hook install via the `Target` registry:
...
│                       a ~1fps IDLE_AMBIENT tick (keeps the clock/weather/pet alive without burning CPU on an
│                       empty office — was a full 0fps freeze); restored [floating] position is validated against
