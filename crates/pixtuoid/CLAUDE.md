...
│                         install-hooks CLI); has_hooks(t) is `pub(crate)` — doctor::diagnose's verify gate is its only
│                         caller since 0.12.0 dropped resolve_connected's install-state migrate inference),
