...
- The JSONL watcher gates historical/ended transcripts on EVERY first-sight path: liveness probe first (CC pid registry / Codex open-rollout FDs), `should_seed_at_eof` fallback. Codex Desktop multiplexes historical open rollouts, so its probe incrementally tracks each rollout's latest structural task boundary and vouches only for running turns; standalone Codex keeps open-FD authority. Probe filesystem work runs off the async source loop. Content NEVER drives lifecycle. The probe also powers ongoing liveness: the `ProofOfLife` sweep exemption, the negative vouch, and the ms-scale `exit_watch` rung.
