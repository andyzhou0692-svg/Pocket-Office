...
- JSONL watcher gates historical/ended transcripts on EVERY first-sight path (initial seed, 250ms rescan, 60s poll, notify) — not just startup, in tier order: the **liveness probe** first (CC's first-party `~/.claude/sessions/<pid>.json` registry, `kill(pid,0)`-verified — probe-live sessions register however stale their mtime), then `should_seed_at_eof` in `walk_jsonl` as the fallback (1-hour mtime window + structural session-end tail scan → seed cursor at EOF, no `SessionStart`). Content-based `/exit` detection is GONE — lifecycle never reads chat content. Unifying this gate was the #85 fix.
- A hook event for an UNKNOWN session id registers it (hooks are proof of life) — blank slot (`#N` label, reap-exempt), back-filled by the next `SessionStart`; JSONL events never synthesize (historical replays).
